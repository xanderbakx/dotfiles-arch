...
E.is_shop = function(){
    return /^(http|https):\/\/hola-shopping.com/i.test(location.href);
};

E.is_be = function(){
    return window.is_tpopup || window.top.be_test_page ||
...
E.require_on_error = function(err){
    ...
    var id = (E.is_shop() ? 'www' : E.is_be() ? 'be_int' : 'www')
    +'_require_err';
...
window.onerror = E.onerror = function(message, file, line, col, err){
    ...
    if (err && err.require_handled)
        return;
    E.perr_once({
        id: (E.is_shop() ? 'www' : E.is_be() ? 'be' : 'www')+'_js_err',
